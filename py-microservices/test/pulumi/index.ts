...
const lambdaPolicy: StackValidationPolicy = {
  ...
  validateStack: async (args, reportViolation) => {
    ...
    if (lambdas.length !== expectedCount) {
      reportViolation(
        `Expected ${expectedCount} lambda function but found ${lambdas.length}`
      );
      return;
    }

    for (let lambdaR of lambdas) {
      const lambda = lambdaR.asType(aws.lambda.Function)!;
...
const dynamoDbPolicy: StackValidationPolicy = {
  ...
  enforcementLevel: "mandatory",
  validateStack: async (args, reportViolation) => {
    const ddbTables = args.resources.filter((r) =>
      r.isType(aws.dynamodb.Table)
    );
    if (ddbTables.length !== 1) {
      reportViolation(
        `Expected one DynamoDB table but found ${ddbTables.length}`
      );
      return;
    }
  },
