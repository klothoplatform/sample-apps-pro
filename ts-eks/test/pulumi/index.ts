    ...
    validateStack: async (args, reportViolation) => {
        ...
        for (const i in nodeGroups) {
            ...
            if (group.nodeGroupName.includes("c4-large")) {
                ...
                group.diskSize !== 100 ?  reportViolation(`Expected disk size to be '100' for 'c4.large' node group, but found ${group.diskSize}`) : null
            } else if (group.nodeGroupName.includes("t3-large")) {
                t3NodeGroupName = group.nodeGroupName
                group.diskSize !== 200 ?  reportViolation(`Expected disk size to be '200' for 't3.large' node group, but found ${group.diskSize}`) : null
            } else {
